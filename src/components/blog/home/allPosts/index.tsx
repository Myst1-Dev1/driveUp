                ...
                <div className="mt-10 grid grid-cols-1 gap-5 lg:gap-0 place-items-center lg:grid-cols-3 2xl:grid-cols-4">
                    {posts?.map(post => (
                        <div key={post.id} className="max-w-96 h-full flex flex-col justify-between w-full">
                            ...
                            <h3 className="text-2xl font-semibold py-3">{post.post_title}</h3>
                            <p className="font-light text-gray-500 text-sm mb-4">{post.post_description}</p>
                            ...
                            <Link href={`/blog/post/${post.id}`} className="w-fit cursor-pointer flex gap-5 items-center font-semibold text-blue-500 py-3 transition-all duration-500 hover:px-3 hover:bg-blue-300 hover:text-white">Saiba Mais <FaLongArrowAltRight className="text-xl" /></Link>
                        </div>
                    ))}
                </div>
