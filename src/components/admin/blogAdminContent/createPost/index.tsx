...
export function CreatePost({ postData }:CreatePostProps) {
    const actionWithData = async (prevState: any, formData: FormData) =>
        createPostAction(prevState, formData, relatedPost);

    const [formState, formAction, pending] = useActionState(actionWithData, { success: false });

    const [file, setFile] = useState<File | null>();
    const [relatedPost, setRelatedPosts] = useState<Posts[]>([]);
