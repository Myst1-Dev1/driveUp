...
export function SideBar() {
    ...
    return (
        <div className="w-0 lg:w-60">
            <div className="py-4 px-5 cursor-pointer z-40 relative" onClick={() => setOpenSideBar(true)}>
                <FaBars className={`${openSideBar ? 'hidden' : 'block'} lg:hidden cursor-pointer transition-all duration-500 hover:text-blue-500`} />
            </div>
            <div className={`fixed z-30 top-0 ${openSideBar ? 'left-0' : '-left-full'} lg:left-0 max-w-60 w-full bg-[#242424] min-h-screen flex flex-col justify-between py-3 transition-all duration-500`}>
